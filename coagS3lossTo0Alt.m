function out = coagS3lossTo0Alt(bins,alpha,d1,d2,rho,T,visc,cmd,ln2s)
%UNTITLED5 Summary of this function goes here
%   Detailed explanation goes here
    
    if  d2/d1 > 3
        out = coagS3lossTo0(bins,alpha,d1,d2,rho,T,visc,cmd,ln2s);
        return;
    end
   
    if d2/d1-1<1e-3
        out=-2*pi*cmd^2*coag_kernel(pi/6*rho*d1^3,pi/6*rho*cmd^3,d1,cmd,T,visc);
        return;
    end
...
%             
%             summa2=summa2+dp2^2*kern*GaussHermiteWeights(j);
%             
%         end
%         
