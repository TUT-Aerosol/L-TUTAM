function out = coagS3gainFrom0Alt(bins,alpha,d1,d2,rho,T,visc,cmd,ln2s)
%UNTITLED5 Summary of this function goes here
%   Detailed explanation goes here
    
    if  d2/d1 > 3
        out = coagS3gainFrom0(bins,alpha,d1,d2,rho,T,visc,cmd,ln2s);
        return;
    end
       
    if d2/d1-1<1e-3
        out=2*pi*(d1^3+cmd^3)^(2/3)*coag_kernel(pi/6*rho*d1^3,pi/6*rho*cmd^3,d1,cmd,T,visc);
        return;
    end
    ...
    [dp2,dp1]=meshgrid(d2Vec,dVec);
    kerroin=(dp1.^3+dp2.^3).^(2/3);
    kern=kern.*kerroin;
...
%             
%             summa2=summa2+(dp1^3+dp2^3)^(2/3)*kern*GaussHermiteWeights(j);
%             
% 
%         end
%         
