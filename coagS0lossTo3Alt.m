function out = coagS0lossTo3Alt(bins,alpha,d1,d2,rho,T,visc,cmd,ln2s)
%UNTITLED5 Summary of this function goes here
%   Detailed explanation goes here
    if  d2/d1 > 3
        out = coagS0lossTo3(bins,alpha,d1,d2,rho,T,visc,cmd,ln2s);
        return;
    end
    
    if d2/d1-1<1e-3
        out=-6.2832*d1^2*coag_kernel(pi/6*rho*d1^3,pi/6*rho*cmd^3,d1,cmd,T,visc);
        return;
    end
...
%             
%             summa2=summa2-dp1^2*kern*GaussHermiteWeights(j);
% 
%         end
%         
